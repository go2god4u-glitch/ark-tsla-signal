...
'6개월 고정' 은 성적은 괜찮지만 원칙이 없다. 진입 규칙과 대칭인 매도 규칙을 찾는다.

  진입: 아크가 많이 샀고(순매수율 상위 10%) 낙폭이 깊을 때(-30% 이하)
  매도: 아크가 그 포지션을 줄였거나(보유량 감소) 낙폭이 회복됐을 때

핵심 지표 두 가지 — 둘 다 진입 조건의 거울상이다:

  아크 보유량 감소  진입 시점 합산 보유 대비 X% 줄면 매도.
...
def main():
    ...
    print("=" * 104)
    print("A. 아크 보유량 감소 — 진입 시점 대비 X% 줄면 매도")
    print("=" * 104)
    ...
    print()
    print("=" * 104)
    print("B. 낙폭 회복 — 낙폭이 -Y% 까지 올라오면 매도 (진입 조건의 거울상)")
    print("=" * 104)
    ...
    print(f"  매도 사유 분포: {dict(Counter(best[1]['why']))}")

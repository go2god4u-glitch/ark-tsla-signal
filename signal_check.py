...
DD_FILTER = -30.0
# [기각된 매수 규칙 B] '극단적 낙폭에서는 아크 문턱을 낮춘다' — 시험했고 버렸다.
#
# 동기는 진짜였다. 규칙 A 만으로는 낙폭이 -48~-54% 로 깊은데 신호가 0건인
# 구간이 있었다(2022-06-03~07-07 최저 -48%, 2025-03-26~04-28 최저 -54%).
# 그래서 낙폭 -45% + 아크 상위 40% 를 규칙 A 와 병렬로 붙여 봤다.
#
# 왜 버렸는가 — 실제 운용 방식(신호마다 100 독립 투입)으로 재면 성적이 내려간다.
#     A 단독   16건  평균 +76.4%  중앙값 +78.0%  승률 94%  최악 -0.8%
#     A | B    28건  평균 +70.2%  중앙값 +66.5%  승률 96%  최악 -0.8%
#     B 추가분 12건  평균 +61.9%
#   B 가 손해를 내는 게 아니라 A 보다 못한 자리에 자본을 1200 더 넣는다.
#   평가액 2822 -> 4764 는 규칙이 좋아진 게 아니라 자본을 1.75배 넣은 결과다.
#
# 왜 격자 검증은 통과했는가 — 격자는 같은 국면의 연속 신호를 1건으로 접어 세고
#   있었다(20건). 실제 시스템은 접지 않는다(28건). 서로 다른 자로 잰 비교였다.
#
# 진짜 원인 — B 문턱(상위 40%)의 중앙값은 0.38% 다. '아크가 조금이라도 사면'
#   참인 조건이라, 낙폭 -45% 국면이 길게 이어지면 **매주** 발화한다.
...
def build(px: pd.Series) -> pd.DataFrame:
    ...
    # 매수 조건은 둘 다 만족해야 한다 — 아크가 강하게 사고(상위 10%),
    # 동시에 값이 충분히 빠져 있을 것(전고점 대비 -30% 이하).

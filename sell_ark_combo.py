"""아크 매도를 청산 규칙에 넣으면 나아지는가 — 조합을 전부 훑는다.

앞서 '아크 매도 단독 청산' 은 0.91배로 최악이었다. 하지만 조합은 시험하지 않았다.
...
def main():
    ...
    print()
    print("=" * 104)
    print("C. 아크 매도 AND RSI 과열 (둘 다 만족해야 청산)")
    print("=" * 104)
    ...
    print()
    print("=" * 104)
    print("D. 아크 매도가 오면 청산하되, 손실 중이면 무시 (이익 실현만)")
    print("=" * 104)
    for q in (0.10, 0.20, 0.30):
        S = set(ark_sell_locs(q))
        def f(e, S=S):
    ...
    print()
    print("=" * 104)
    print("E. RSI 이탈 우선, 아크 매도는 보조 (RSI 미도달 시에만 아크로 청산)")
    print("=" * 104)
    for q in (0.10, 0.20, 0.30):

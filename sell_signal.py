...
두 질문을 나눈다. 섞으면 답이 안 나온다.
  ...
  B. 매도 규칙으로서 6개월 보유를 이기는가
     진입은 아크 매수 신호로 고정하고 매도 규칙만 바꿔 비교한다.
     A 에서 예측력이 있더라도, 매수 신호와 매도 시점이 맞물리지 않으면
...
규율은 매수 분석과 동일하다.
  ...
  - 진입·매도 모두 신호 다음 거래일 종가 (공시는 장 마감 후)
  - 기준은 같은 기간에서만 추출
  - 시험한 규칙을 전부 출력한다
"""
...
NB = 20000
HOLD_CAP = 504             # 규칙이 안 걸리면 최대 2년에서 강제 매도
...
def simulate(V, entries, exit_locs_sorted, cap=HOLD_CAP):
    """진입 후 첫 번째 매도 신호에서 판다. 없으면 cap 에서 강제 매도."""
...
def main() -> None:
    ...
    print(f"\n{'매도 규칙':<28s}{'매매':>5s}{'평균':>9s}{'중앙':>9s}{'승률':>6s}"

"""신호마다 독립 포지션 — 매 신호에 100 을 넣고, 각자의 청산 시계를 돈다.

자본 배분 문제를 없앤 방식이다. 신호가 뜰 때마다 새로 100 을 투입하고,
그 포지션은 자기 진입일 기준으로 청산 조건을 본다. 이미 보유 중이어도 상관없다.

  진입  아크 주간 신호 -> 다음 거래일 종가, 100 투입
  청산  그 포지션 진입 이후 RSI(14) 가 70 을 넘었다가 다시 70 아래로
...
def main():
    ...
    print(f"신호 {len(rows)}건 — 각각 100 투입, 각자의 청산 시계\n")
    print(f"  {'진입':<12s}{'진입가':>9s}{'청산':<14s}{'청산가':>9s}{'보유':>6s}{'수익':>9s}  사유")
    for r_ in rows:
        e = r_['entry']
        if r_['open']:
